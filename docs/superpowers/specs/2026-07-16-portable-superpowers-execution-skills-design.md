...
`using-git-worktrees` is required by both execution workflows. The complete
`subagent-driven-development` payload includes its implementer, specification
reviewer, and code-quality reviewer prompt files; no file will be copied from
Codex's mutable marketplace cache.
...
expanded audited subset. The full Superpowers plugin remains disabled.
...
skills and continue validating every tracked TOML, agent, hook, and skill.
