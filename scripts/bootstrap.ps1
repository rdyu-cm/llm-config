...
function Install-MergedConfig {
    ...
    if (Test-Path -LiteralPath $target) {
        ...
        if ($item.LinkType) {
            ...
            if ((Get-Content -LiteralPath $target -TotalCount 1) -eq "# Generated by scripts/sync_config.py; do not edit directly.") {
                $managedActive = $true
            } else {
    ...
    if ($Apply) {
        if ($managedActive -and -not (Get-Item -LiteralPath $target -Force).LinkType) {
            Copy-Item -LiteralPath $target -Destination $generated -Force
        }
        & $Python $sync --base $base --local $mergeLocal --output $generated
        ...
        Write-Host "would   install merged config at $target"
    }
}
