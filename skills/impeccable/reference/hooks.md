...
Supported harness: Claude Code (`.claude/settings.local.json` in the project, which is gitignored so the hook stays machine-local; a hook moved into shared `.claude/settings.json` is honored in place too).
...
- The hook is bundled with the Impeccable skill and installed through `.claude/settings.local.json` or shared `.claude/settings.json`.
